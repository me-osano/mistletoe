if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcpio.conf.d/mistletoe_hooks.conf <<EOF >/dev/null
...
EOF
  ...
  sudo cp $MISTLETOE_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $MISTLETOE_PATH/default/limine/limine.conf /boot/limine.conf


  # Match Snapper configs if not installing from the ISO
  if [[ -z ${MISTLETOE_CHROOT_INSTALL:-} ]]; then
...
#
#   uki_file=$(find /boot/EFI/Linux/ -name "mistletoe*.efi" -printf "%f\n" 2>/dev/null | head -1)
#
#   if [[ -n "$uki_file" ]]; then
